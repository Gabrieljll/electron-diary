...
function registerIpcHandlers() {
    // Productos
    ipcMain.handle('producto:nuevo', async (_, fichaCliente) => {
        try {
            ...
            showNotification('Pombero Stock', 'Nuevo Producto Agregado!');
    ...
    ipcMain.handle('producto:obtener-todos', async () => {
        const conn = await getConnection();
        const [fichas] = await conn.query('SELECT * FROM stock_productos ORDER BY id DESC');
        return fichas;
    });

    ipcMain.handle('producto:borrar', async (_, id) => {
        const conn = await getConnection();
        try {
    ...
    ipcMain.handle('producto:obtener-por-id', async (_, id) => {
        const conn = await getConnection();
        const [result] = await conn.query('SELECT * FROM stock_productos WHERE id = ?', [id]);
        return result[0];
    });

    ipcMain.handle('producto:actualizar', async (_, id, producto) => {
        const conn = await getConnection();
    ...
    ipcMain.handle('producto:actualizar-stock', async (_, id, datos) => {
        const conn = await getConnection();
        await conn.query(
    ...
    // Ventas
    ipcMain.handle('venta:obtener-por-fecha', async (_, fecha) => {
        const conn = await getConnection();
    ...
    ipcMain.handle('venta:registrar', async (_, { productos, cliente, telefono, direccion, costoEnvio, metodoPago, total, fecha }) => {
        try {
            const conn = await getConnection();
            await conn.beginTransaction();

            // Crear orden
            const [ordenResult] = await conn.query('INSERT INTO ordenes (fecha) VALUES (NOW())');
            ...
            for (const item of productos) {
                if (item.tipo === 'producto') {
                    ...
                    for (const detalle of detalles) {
                        const cantidadTotal = detalle.cantidad * item.cantidad;
                        await conn.query(
                            `UPDATE stock_productos SET cantidad_disponible = cantidad_disponible - ? WHERE id = ?`,
            ...
            await conn.query(
                `INSERT INTO venta_producto (id_orden, nombre_cliente, telefono, direccion, costo_envio, modo_pago, total, fecha) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [idOrden, cliente, telefono, direccion, costoEnvio, metodoPago, total, fecha]
            );
            ...
            if (clienteExistente && clienteExistente.cantidad_compras !== null) {
                ...
                await conn.query(
                    ...
                    [nuevaCantidad, cliente, telefono]
                );
            } else {
                await conn.query(
            ...
            await conn.commit();
            showNotification('Pombero Stock', 'Venta registrada exitosamente!');
            return { success: true };
    ...
    ipcMain.handle('venta:eliminar', async (_, idVenta) => {
        try {
            const conn = await getConnection();
            ...
            showNotification('Pombero Ventas', 'Venta eliminada exitosamente!');
            return { success: true };
        } catch (error) {
            showNotification('Pombero Stock', 'Ocurrió un error al tratar de eliminar el registro!');
            throw error;
        }
    });

    ipcMain.handle('venta:obtener-por-mes-anio', async (_, anio, mes, tipo = 'total') => {
        const conn = await getConnection();
    ...
    // Combos
    ipcMain.handle('combo:nuevo', async (_, combo, detalles) => {
        const conn = await getConnection();
        ...
        try {
            ...
            await conn.commit();
            showNotification('Pombero Stock', 'Nuevo Combo Agregado!');
    ...
    ipcMain.handle('combo:obtener-todos', async () => {
        const conn = await getConnection();
    ...
    ipcMain.handle('combo:obtener-por-id', async (_, idCombo) => {
        const conn = await getConnection();
    ...
    ipcMain.handle('combo:actualizar', async (_, idCombo, combo, detalles) => {
        const conn = await getConnection();
        ...
        try {
            ...
            await conn.commit();
            showNotification('Pombero Stock', 'Combo Actualizado!');
            return { success: true };
    ...
    ipcMain.handle('combo:borrar', async (_, idCombo) => {
        const conn = await getConnection();
        ...
        try {
            ...
            await conn.commit();
            showNotification('Pombero Stock', 'Combo Eliminado!');
            return { success: true };
    ...
    // Clientes
    ipcMain.handle('cliente:buscar-por-nombre', async (_, parteNombre) => {
        const conn = await getConnection();
        const [resultado] = await conn.query(
    ...
    // Reportes
    ipcMain.handle('reporte:ganancias-dia', async (_, fecha) => {
        const conn = await getConnection();
    ...
    ipcMain.handle('reporte:compras-realizadas', async () => {
        const conn = await getConnection();
    ...
    // Configuración
    ipcMain.handle('config:obtener-recargos', async () => {
        try {
            const conn = await getConnection();
    ...
    ipcMain.handle('config:guardar-recargos', async (_, credito, debito) => {
        try {
            const conn = await getConnection();
    ...
    ipcMain.handle('config:guardar-descuentos', async (_, credito, debito) => {
        return true;
    });
}
